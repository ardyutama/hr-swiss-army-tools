...
Research question: how to install hr-sat (ASP.NET Core 10 Kestrel Web API + Vue 3 SPA + EF Core/Npgsql/PostgreSQL + on-disk file storage) on Windows as a desktop-native app with an easy, ideally zero-config, setup — for (a) fully-local single-machine use by an HR person and (b) on-prem deployment.

**TL;DR — recommended default path:** Publish the Web.Api as a self-contained single-file exe (`dotnet publish -r win-x64`) that serves the built Vue SPA as static files, launched as a per-user console/tray app bound to a dynamic localhost port. **No .NET runtime prerequisite**; fully verified against Microsoft Learn.

For the database, use **SQLite via `Microsoft.EntityFrameworkCore.Sqlite` for the fully-local single-user desktop case** (zero-config, zero-service; the codebase's only Npgsql-specific runtime code is one `FOR UPDATE` raw SQL in `AppDbContext`), and keep **PostgreSQL for on-prem multi-user** (central server, Windows Service or Docker).

Ship with **Velopack** — one-click Setup.exe into `%LocalAppData%`, no admin, delta updates, active maintenance — rather than MSIX unless enterprise IT demands MSIX/Intune. MSIX adds a mandatory code-signing trust hurdle, and its one-click web-install protocol has been disabled by default on consumer devices since Dec 2023.

Apply EF migrations at app startup (`MigrateAsync()`) with a file-copy backup beforehand for SQLite; migration locking (EF Core 9+) makes startup migration safe for single-instance desktop use. The long-term on-prem path (same codebase as Windows Service/Docker) stays open because provider selection is a one-liner in `Program.cs` — keep it config-driven.

Caveats: NativeAOT is **not** a realistic option for this codebase (EF Core + Npgsql rely on reflection; ASP.NET Core NativeAOT explicitly excludes MVC and several middleware, and only partially supports Minimal APIs). Embedding PostgreSQL via `MysticMind.PostgresEmbed` is technically possible but the package was last updated 2023-12 (verified on nuget.org), so treat it as "spike at your own risk". All version/status statements verified against primary sources on 2026-09-15; items marked *(inference)* are reasoning from those sources, not directly sourced claims.
...
- Npgsql-specific surface (grep of `src/hr-sat.Infrastructure`, excluding `bin/obj`):
  ...
  - No `jsonb`, `citext`, tsvector/full-text search, or `ILike` usage found in code (ADR-0009 full-text matching is planned but not implemented in Infrastructure at time of writing).
...
`dotnet publish` supports single-file output for both framework-dependent and self-contained apps; self-contained bundles the runtime and needs **no .NET install** on the target. Single-file is always RID-specific (e.g. `win-x64`), and only managed DLLs are bundled by default — native libraries (relevant if Sdcb.PaddleOCR ships native assets) ship as separate files unless `IncludeNativeLibrariesForSelfExtract=true` is set, in which case they extract to `%TEMP%/.net` on Windows. Optional `EnableCompressionInSingleFile=true` shrinks the exe at a startup cost. ([Microsoft Learn — single-file deployment](https://learn.microsoft.com/en-us/dotnet/core/deploying/single-file/overview))

Known API caveats: `Assembly.Location` returns an empty string, `Assembly.CodeBase` throws — use `AppContext.BaseDirectory` for files next to the exe ([same page](https://learn.microsoft.com/en-us/dotnet/core/deploying/single-file/overview#api-incompatibility)). hr-sat's `PrivateFileStorage` and any "files next to exe" logic must use `AppContext.BaseDirectory` or better, `%LocalAppData%`.

For the SPA: `StaticFiles` middleware is fully compatible — the [ASP.NET Core Native AOT compatibility table](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/native-aot) lists StaticFiles as supported even under AOT. hr-sat would add `MapStaticAssets()`/`UseStaticFiles()` + `MapFallbackToFile("index.html")` in `Program.cs` and copy `wwwroot` from the Vite build at publish time (standard pattern, *(inference)*). The `Spa` *middleware package* (`Microsoft.AspNetCore.SpaProxy`-style dev-time integration) is listed as **not supported** under NativeAOT, but that is irrelevant here because dev proxying is via Vite.

Port strategy: `UseUrls("http://127.0.0.1:0")` binds a dynamic port, which avoids conflicts; launch the browser (or shell) at the bound address after startup, *(inference)*. Kestrel endpoint configuration via `ASPNETCORE_URLS` or config is documented in the [Windows Service hosting article](https://learn.microsoft.com/en-us/aspnet/core/host-and-deploy/windows-service?view=aspnetcore-10.0#configure-endpoints) and [Kestrel endpoints](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/servers/kestrel/endpoints).
...
- **Query:** `DateTimeOffset`, `decimal`, `TimeSpan`, `ulong` aren't natively supported — equality works, but comparisons and ordering evaluate client-side; docs recommend `DateTime` + UTC and `double` (or a value converter) instead of `decimal`.
- **Migrations:** several schema ops require a table **rebuild** (AddForeignKey, AlterColumn, DropColumn, etc. — full list in the doc); **idempotent migration scripts are not supported** on SQLite.
...
4. **The `FOR UPDATE` lock:** how should `FindVacancyForUpdateAsync` behave on SQLite (no-op read inside transaction vs optimistic concurrency token)? Decide when the SQLite provider lands.
...
8. **OCR payload (ADR-0009):** if Sdcb.PaddleOCR ships, native assets must be excluded from single-file bundling or handled via `IncludeNativeLibrariesForSelfExtract`; decide the publish profile then.
