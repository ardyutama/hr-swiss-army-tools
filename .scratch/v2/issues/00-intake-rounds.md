...
- [x] Vacancy owns rounds; candidate FK moves from vacancy to round; EF migration moves
  existing candidates into a default Round 1 per vacancy
- [x] Create round (only when none active), close round (manual, irreversible, confirm
  when vacancy still has shortage per ticket 01 — plain confirm for now), closed rounds
  read-only (409 on writes)
- [x] Candidate import targets the active round; rejected (409) when no round is active
- [x] Review workspace re-homes to `/vacancies/:id/rounds/:roundId/review/:candidateId`;
  candidate list, import dialog, filters, and composables key off round
- [x] Vacancy-detail = header + round list (row: "Round N — name", open/closed, candidate
  count) + active round's candidate list; closed rounds reachable read-only
- [x] Quiet UI: single open round keeps the round list hidden and exposes a subtle
  `Manage rounds` entry point; the V1 candidate view remains otherwise untouched
